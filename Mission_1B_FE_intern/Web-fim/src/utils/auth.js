...
export function loginUser({ usernameOrEmail, password }) {
  ...
  if (!found) return { ok: false, message: "Username/email atau password salah" };
  // simpan session
  localStorage.setItem("session", JSON.stringify({ username: found.username, email: found.email }));
  return { ok: true, user: { username: found.username, email: found.email } };
}
