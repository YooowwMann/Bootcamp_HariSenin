  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-white">
      ...
      <div className="relative w-full h-[500px]">
        <img
          src="https://image.tmdb.org/t/p/w500/vUUqzWa2LnHIVqkaKVlVGkVcZIW.jpg"
        ...
        <div className="absolute bottom-16 left-12 max-w-xl">
          <h1 className="text-4xl font-bold mb-4">Drama Thriller Jerman</h1>
          <p className="text-gray-300 mb-6">
            Kisah menegangkan tentang bertahan hidup dari ancaman mafia yang terjadi di sebuah kota.
            Ketegangan meningkat ketika setiap penghuni mulai menunjukkan sisi gelapnya.
          </p>
      ...
      <div className="px-10 py-12 space-y-12">
        <SeriesRow title="Melanjutkan Tonton Series" data={safeSlice(popularSeries, 5, 12)} />
        <SeriesRow title="Series Persembahan Chill" data={safeSlice(popularSeries, 0, 40)} />
        <SeriesRow title="Top Rating Series Hari Ini" data={popularSeries.filter((f) => Number(f.rating) > 9).sort((a, b) => Number(b.rating) - Number(a.rating)).slice(0, 29)} />
        <SeriesRow title="Series Trending" data={safeSlice(popularSeries, 15, 29)} />
        <SeriesRow title="Rilis Baru" data={popularSeries.filter((f) => Number(f.year) > 2020).sort((a, b) => Number(b.year) - Number(a.year)).slice(0, 29)} />
      </div>
