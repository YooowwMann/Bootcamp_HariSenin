...
const Film = ({searchQuery, onSearch}) => {
  const [films] = useState(popularFilms);
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-white">
      ...
      <div className="relative w-full h-[500px]">
        <img
          src="https://image.tmdb.org/t/p/w500/fSRb7vyIP8rQpL0I47P3qUsEKX3.jpg"
        ...
        <div className="absolute bottom-16 left-12 max-w-xl">
          <h1 className="text-4xl font-bold mb-4">Deadpool: Aksi Tanpa Batas</h1>
          <p className="text-gray-300 mb-6">
            Aksi brutal, humor sarkas, dan petualangan yang tidak bisa ditebak.
            Deadpool kembali beraksi melawan ancaman yang lebih gila dari sebelumnya.
          </p>
      ...
      <div className="px-10 py-12 space-y-12">
        <FilmRow title="Melanjutkan Tonton Film" data={safeSlice(popularFilms, 0, 10)} />
        <FilmRow title="Film Persembahan Chill" data={safeSlice(popularFilms, 0, 23)} />
        <FilmRow title="Top Rating Film Hari Ini" data={popularFilms.filter((f) => Number(f.rating) > 8.9).sort((a, b) => Number(b.rating) - Number(a.rating)).slice(0, 23)} />
        <FilmRow title="Film Trending" data={safeSlice(popularFilms, 10, 20)} />
        <FilmRow title="Rilis Baru" data={popularFilms.filter((f) => Number(f.year) > 2020).sort((a, b) => Number(b.year) - Number(a.year)).slice(0, 23)} />
      </div>
      </>)}
